var tableId = jsUrls.SeatTableId;
var modalId = jsUrls.SeatModalId;
...
var InitializeListData = function () {
    ...
    var ajaxObject = {
        "url": jsUrls.LoadSeatList,
        "type": "POST",
        "data": function (d) {
            var pageNumber = $('#' + tableId).DataTable().page.info();
            d.PageNumber = pageNumber.page;

            d.SeatTableFilter = $('#' + tableId + 'Filter').val();
    ...
    var columnObject = [
        {
            "data": "seatId", "name": "seatId", "autowidth": true, "render": function (data, type, row, meta) {
                if (data != 0) {
                    return "<span>" + row.seatId + "</span>";
                } else {
                    return "";
                }
            }
        ...
        {
            "data": "seatCapacity", "name": "SeatCapacity", "autoWidth": true, "render": function (data, type, row, meta) {
                if (data != 0) {
                    return "<span>" + row.seatCapacity + "</span>";
                } else {
                    return "";
                }
            }
        ...
        },
        {
            "data": "Action", "name": "Action", "render": function (data, type, row, meta) {
                var html = '';
                html += "<span style='display:none;'>" + row.seatId + "</span>";

                var deleteEvent = "ConfirmDelete('" + row.seatId + "')";
                ...
                var editEvent = "ManageView('" + row.seatId + "')";
...
var ManageView = function (id) {
    var url = jsUrls.InsertUpdateSeat + "?id=" + id;
    MakeAjaxCall("GET", url, null, true, tableId, modalId);
}

var ConfirmDelete = function (SeatId) {
    if (SeatId != 0)
        $('#hdnDelSeatId').val(SeatId);
    var url = jsUrls.DeleteSeat + "?id=" + SeatId;
    MakeAjaxCall("GET", url, null, true, tableId, modalId);
}

$('#btnAddSeat').click(function () {
    var url = jsUrls.InsertUpdateSeat;
    MakeAjaxCall("GET", url, null, true, tableId, modalId);
});